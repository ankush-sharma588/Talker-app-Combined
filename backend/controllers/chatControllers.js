...
const accessChat = asyncHandler(async (req, res) => {
  ...
  if (!userId) { console.log("UserId param not sent with request"); return res.sendStatus(400); }
...
const removeFromGroup = asyncHandler(async (req, res) => {
  ...
  const removed = await Chat.findByIdAndUpdate(chatId, { $pull: { users: userId } }, { new: true })
    .populate("users", "-password").populate("groupAdmin", "-password");
  if (!removed) { res.status(404); throw new Error("Chat Not Found"); }
...
const addToGroup = asyncHandler(async (req, res) => {
  ...
  const added = await Chat.findByIdAndUpdate(chatId, { $push: { users: userId } }, { new: true })
    .populate("users", "-password").populate("groupAdmin", "-password");
  if (!added) { res.status(404); throw new Error("Chat Not Found"); }
  else res.json(added);
});
...
const deleteChat = asyncHandler(async (req, res) => {
  ...
  // Only a member can delete the chat
  const isMember = chat.users.some((u) => u.toString() === req.user._id.toString());
  if (!isMember) { res.status(403); throw new Error("Not authorized to delete this chat"); }
