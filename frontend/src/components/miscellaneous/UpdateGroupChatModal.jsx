...
const UpdateGroupChatModal = ({ fetchMessages, fetchAgain, setFetchAgain }) => {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      toast({ title: "Error Occurred!", description: "Failed to load search results", status: "error", duration: 5000, isClosable: true, position: "bottom-left" });
      setLoading(false);
    }
  };
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await api.put(`/api/chat/rename`, { chatId: selectedChat._id, chatName: groupChatName }, config);
  ...
  const handleAddUser = async (user1) => {
    ...
    try {
      setLoading(true);
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await api.put(`/api/chat/groupadd`, { chatId: selectedChat._id, userId: user1._id }, config);
      setSelectedChat(data);
      setFetchAgain(!fetchAgain);
      setLoading(false);
    } catch (error) {
  ...
  const handleRemove = async (user1) => {
    ...
    try {
      setLoading(true);
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await api.put(`/api/chat/groupremove`, { chatId: selectedChat._id, userId: user1._id }, config);
  ...
  return (
    <>
      <IconButton icon={<ViewIcon />} onClick={onOpen} />  {/* Chakra v2: d prop removed */}
      ...
      <Modal onClose={onClose} isOpen={isOpen} isCentered>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody display="flex" flexDir="column" alignItems="center">   {/* d → display */}
            <Box w="100%" display="flex" flexWrap="wrap" pb={3}>            {/* d → display */}
              {selectedChat.users.map((u) => (
                <UserBadgeItem key={u._id} user={u} admin={selectedChat.groupAdmin} handleFunction={() => handleRemove(u)} />
              ))}
            </Box>
            <FormControl display="flex">                                     {/* d → display */}
              <Input
